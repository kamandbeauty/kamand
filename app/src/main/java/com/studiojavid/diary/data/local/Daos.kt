...
interface DiaryDao {
    ...
    fun observeFavorites(): Flow<List<DiaryEntity>>

    @Query("SELECT * FROM memories WHERE date = :epochDay")
    fun observeByDate(epochDay: Long): Flow<DiaryEntity?>

    @Query("SELECT * FROM memories WHERE date = :epochDay")
    suspend fun findByDate(epochDay: Long): DiaryEntity?
    ...
    fun observeBetween(from: Long, to: Long): Flow<List<DiaryEntity>>
    ...
    fun observeOnDays(epochDays: List<Long>): Flow<List<DiaryEntity>>
    ...
    suspend fun insert(page: DiaryEntity): Long

    @Update
    suspend fun update(page: DiaryEntity)
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertAll(memories: List<DiaryEntity>)
}
