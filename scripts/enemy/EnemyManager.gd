...
func spawn_enemy(position: Vector3, target: Node3D = null, provider: Node = null) -> PrototypeEnemy:
 if pool.is_empty(): return null
 var enemy: PrototypeEnemy = pool.pop_back(); enemy.global_position = position; enemy.setup(target, provider); active.append(enemy); enemy.defeated.connect(despawn_enemy.bind(enemy), CONNECT_ONE_SHOT); return enemy
