...
### نسخهٔ ۶ — رهگیری ارسال

- جدول جدید `shipments`: `order_id` (یکتا)، `tracking_code` (TEXT — رشته‌ای)، `provider` (پست/تیپاکس/پیک/سایر)، `shipped_at`
- پیوند از طریق **سفارش** (نه مشتری) — سفارش‌های یک مشتری رکوردهای رهگیری جدا دارند
- **هیچ فیلد آدرسی وجود ندارد**
- ذخیرهٔ گروهی تراکنشی؛ ردیف نامعتبر = ذخیرهٔ هیچ؛ ردیف‌های دست‌نخورده تغییر نمی‌کنند؛ وضعیت سفارش به SHIPPED به‌روز می‌شود
