...
> **توجه:** محیط سندباکسِ توسعه به مخازن Google Maven / Gradle دسترسی نداشت، بنابراین بیلد نهایی باید روی ماشین شما یا GitHub Actions اجرا شود. فایل ورک‌فلوی آماده در `ci/memory-android.yml` قرار دارد — کافی است آن را در `.github/workflows/` قرار دهید (توکن ربات اجازهٔ ساخت فایل workflow را نداشت).
>
...
### پیش‌نمایش طراحی
`memory/preview/index.html` یک پیش‌نمایش زندهٔ HTML از هویت بصری اپ است (همان توکن‌های رنگ، فونت، تقویم شمسی و اعداد فارسی) که بدون نیاز به اندروید استودیو در مرورگر باز می‌شود.
...
./gradlew :app:assembleRelease \
  -PapplicationId=com.yourcompany.memory \
  -PappName="خاطرات"
```

یا به‌صورت دائمی در `app/build.gradle.kts` مقادیر پیش‌فرض `memoryApplicationId` و `memoryAppName` را عوض کنید.
...
```bash
keytool -genkey -v -keystore memory.jks -keyalg RSA \
  -keysize 2048 -validity 10000 -alias memory
```

سپس فایل `memory/keystore.properties` را بسازید:

```properties
storeFile=memory.jks
storePassword=******
keyAlias=memory
keyPassword=******
```
...
- **MVVM** با `ViewModel` + `StateFlow`؛ بدون فریم‌ورک DI (سرویس‌لوکیتور سبک در `MemoryApp`).
...
فرمت `memory-backup` v1 عمداً transport-agnostic است تا افزودن Cloud Backup در آینده فقط جابه‌جایی همین بایت‌ها باشد.
