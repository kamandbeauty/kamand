...
import com.studiojavid.memory.data.local.DayMark
...
class MemoryRepository(
    ...
    val allPages: Flow<List<MemoryPage>> = dao.observeAll().map { it.toDomain() }

    val favorites: Flow<List<MemoryPage>> = dao.observeFavorites().map { it.toDomain() }
    ...
    fun pageOn(date: LocalDate): Flow<MemoryPage?> =
        dao.observeByDate(date.toEpochDay()).map { it?.toDomain() }

    fun pagesBetween(from: LocalDate, to: LocalDate): Flow<List<MemoryPage>> =
     ...
     */
    fun onThisDay(today: LocalDate = LocalDate.now(), yearsBack: Int = 10): Flow<List<MemoryPage>> {
    ...
    suspend fun restorePage(page: MemoryPage) {
        dao.insert(
            MemoryEntity(
                id = page.id,
    ...
    suspend fun snapshot(): List<MemoryEntity> = dao.observeAll().first()

    suspend fun replaceAll(pages: List<MemoryEntity>) {
        dao.clear()
        dao.insertAll(pages)
        pruneOrphanPhotos()
    }
    ...
    private companion object {
        const val PHOTO_DIR = "memory_photos"
    }
}

private fun List<MemoryEntity>.toDomain(): List<MemoryPage> = map { it.toDomain() }

private fun MemoryEntity.toDomain() = MemoryPage(
    id = id,
