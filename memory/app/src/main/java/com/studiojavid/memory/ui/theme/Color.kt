 ...
 */
fun memoryColorsOf(palette: ThemePalette, dark: Boolean): MemoryColors {
    val p = if (dark) palette.dark else palette.light
    val base = if (dark) DarkMemoryColors else LightMemoryColors
    return base.copy(
        isDark = dark,
 ...
 */
fun MemoryColors.timeOfDayGradient(hour: Int): List<Color> {
    val warm: Color
    val cool: Color
    when (hour) {
