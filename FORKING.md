...
چون این اپ هرگز منتشر نشده بود، هیچ دیتابیس نصب‌شده‌ای برای مهاجرت وجود نداشت
...
وقتی اپ به بلوغ رسید و باید مستقل منتشر شود، `tools/extract_memory_branch.sh`
پوشهٔ `memory/` را به ریشهٔ یک برنچ تازه می‌برد (`app/`، `ci/`، `tools/`) با یک
کامیت بدون والد.

```bash
git fetch origin
git switch arena/01a002a3-kamand   # برنچی که اپ رویش است
bash tools/extract_memory_branch.sh
git push -u origin memory-app
```

اسکریپت برنچ مبدأ را فقط از طریق `git worktree` می‌خوانَد و هرگز رویش
نمی‌نویسد؛ همه‌چیز در پوشهٔ موقت ساخته می‌شود و **قبل از ساختن برنچ** هر دو
بررسی ایستا روی خروجی اجرا می‌شوند. پوش پیش‌فرض خاموش است (`--push` برای
یک‌مرحله‌ای شدن).

بدون گیت روی کامپیوتر هم می‌شود: `memory/ci/publish-memory-branch.yml` را در
`.github/workflows/` بگذارید و از تب Actions اجرا کنید — همین اسکریپت را روی
سرور گیت‌هاب اجرا می‌کند، پس دو مسیر هرگز از هم واگرا نمی‌شوند.
