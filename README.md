...
> نسخهٔ فعلی: **۱.۰.۸** · شناسهٔ اندروید: `com.ruby.factor_ruby` · زبان رابط: فارسی
...
| مهاجرت و به‌روزرسانی | `MIGRATION.md` | به‌روزرسانی نسخه‌ی نصب‌شده بدون حذف نصب و بدون از دست رفتن داده |
...
## به‌روزرسانی نسخه‌ی نصب‌شده (بدون حذف نصب)

کاربران فعلی روبی لازم نیست برنامه را حذف و دوباره نصب کنند. نسخه‌ی تازه با
همان `applicationId` و همان کلید امضا منتشر می‌شود و هنگام اولین اجرا، داده‌های
نسخه‌ی قبلی (فاکتورها، مشتریان، کالاها، تنظیمات و پیش‌نویس‌ها) به ساختار تازه
منتقل می‌شوند:

- پیش از هر تغییر، یک پشتیبان خودکار روی گوشی ساخته می‌شود.
- مهاجرت گام‌به‌گام و قابل‌ازسرگیری است (`lib/core/migration/`).
- هیچ رکوردی حذف نمی‌شود و هیچ مبلغی دو بار حساب نمی‌شود.
- برای فاکتورهای قدیمی، سود و بهای تمام شده و کارت حساب تامین‌کنندگان
  بازسازی می‌شود.
- مشتری‌هایی که در فهرست نیستند اما فاکتور دارند، از روی فاکتورهایشان
  بازسازی می‌شوند (بازیابی اطلاعاتی که در نسخه‌های قبلی از دست می‌رفت).

جزئیات کامل، تضمین‌ها و روش بازیابی در [`MIGRATION.md`](MIGRATION.md) آمده است.
بررسی خودکار سازگاری به‌روزرسانی با `scripts/verify_upgrade_safety.sh` انجام
می‌شود و در CI روی هر بیلد اجرا می‌گردد.
