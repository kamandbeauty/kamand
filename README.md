...
**مدیریت سفارشات و حسابداری جاوید** (انتشار قبلی: فاکتور روبی) — اپلیکیشن اندرویدی فارسی و کاملاً آفلاین برای مدیریت کامل کسب‌وکارهای کوچک: صدور فاکتور فروش و خرید، انبار و موجودی، مشتریان و اعتبار، چک، هزینه، صندوق و بانک، فروش اقساطی با کارمزد درگاه‌ها، سفارشات ارسال‌نشده و گزارش‌های مالی حرفه‌ای.

> نسخهٔ فعلی: **۱.۰.۴** · شناسهٔ اندروید: `com.javid.accounting` · ناشر: **Studio Javid** · زبان رابط: فارسی (RTL)

## امکانات کامل

### 🧾 فاکتور فروش و پیش‌فاکتور
- فاکتور فروش، پیش‌فاکتور و تبدیل پیش‌فاکتور به فروش
- اقلام از کاتالوگ یا ثبت دستی · تخفیف مبلغی/درصدی · هزینهٔ ارسال · بیعانه · بدهی قبلی
- **مالیات بر ارزش افزوده**: با تیک فعال می‌شود؛ درصد دلخواه تایپ می‌شود؛ به مبلغ نهایی اضافه و در پیش‌نمایش فاکتور و کل حسابداری محاسبه می‌شود
- شمارهٔ فاکتور یکتا بین فاکتورهای فعال · حذف نرم با حفظ تاریخ مالی
- پیش‌نمایش چاپی، تصویر فاکتور، اشتراک‌گذاری و ذخیره در گالری
- تب‌های موقت برای چند فاکتور باز هم‌زمان

### 🛒 فاکتور خرید
- تأمین‌کننده، اقلام با قیمت خرید، تخفیف، ارسال، سایر هزینه‌ها
- **مالیات ارزش افزوده با تیک و درصد دلخواه**
- ورود خودکار موجودی با بهای میانگین موزون · پرداخت همراه خرید (نقد/نسیه)
- برگشت خرید با سقف مقدار خریدشده (کاهش موجودی و بدهی)

### 📦 محصولات، انبار و موجودی
- کاتالوگ کالا/خدمت: کد، واحد، قیمت خرید و فروش، موجودی
- **موجودی منفی مجاز**: فروش کالای ناموجود موجودی را منفی می‌کند و خرید بعدی جبران می‌شود — همیشه «موجودی = جمع حرکت‌ها»
- تاریخچهٔ حرکت موجودی، تعدیل شفاف، حداقل موجودی و هشدار کمبود، ارزش‌گذاری انبار
- گزارش سود هر کالا (تعداد/درآمد/بهای/حاشیه/برگشتی)

### 👥 مشتریان
- بانک مشتریان با نام/شماره/آدرس/یادداشت · جست‌وجو
- **نام و شمارهٔ مشتریان از سفارشات هم خودکار ذخیره می‌شود**
- ماندهٔ بدهی همیشه مشتق از دفتر کل · سقف اعتبار برای اقساط مستقیم
- صورت‌حساب کامل، دریافت (نقد/کارت‌به‌حساب/**چک**)، برگشت وجه، شارژ بسته‌بندی/ارسال

### 📮 کدهای رهگیری ارسال
- صفحهٔ عملیاتی «کدهای رهگیری ارسال»: هر سفارش یک ردیف (نام مشتری + شمارهٔ فاکتور + کد رهگیری + تاریخ ارسال + روش ارسال + وضعیت) — بدون هیچ فیلد آدرس
- **ورود گروهی**: ثبت کد رهگیری ده‌ها/صدها سفارش در یک صفحه و ذخیره با یک دکمهٔ «ثبت اطلاعات ارسال» (تراکنشی — ردیف نامعتبر مانع کل ذخیره می‌شود و ردیف‌های دست‌نخورده تغییر نمی‌کنند)
- کد رهگیری رشته‌ای است — صفر ابتدایی، حروف و خط تیره عیناً حفظ می‌شوند
- جست‌وجوی نام مشتری + فیلتر (همه / ارسال‌نشده / بی‌کد / ارسال‌شده) در سطح دیتابیس؛ ترتیب هوشمند (اول نیازمندان کد)
- با ثبت کد رهگیری، سفارش خودکار به «ارسال‌شده» منتقل می‌شود
- «ارسال کد رهگیری»: پیام آمادهٔ هر سفارش + پیامک (باز شدن پیامک‌رسان اندروید) + اشتراک‌گذاری بومی (واتساپ/تلگرام/بله/ایتا/…) + اشتراک گروهی چند سفارش
- ثبت رهگیری فقط عملیاتی است — هیچ رویداد مالی/موجودی/مبلغی تغییر نمی‌کند

### 📦 سفارشات ارسال‌نشده
- فهرست سفارش‌های در انتظار ارسال · ضربه روی هر سفارش → جزئیات کامل
- ویرایش کامل (مشتری، اقلام، تخفیف، ارسال، توضیحات) · علامت «ارسال شد» / لغو
- ذخیرهٔ خودکار نام و شمارهٔ مشتری در بانک مشتریان

### 🤝 تأمین‌کنندگان
- پروفایل کامل + فعال/غیرفعال · بدهی مشتق از دفتر کل
- پرداخت نقدی یا **چک** · صورت‌حساب · گزارش خرید/پرداخت

### 💳 چک‌ها
- دریافت چک از مشتری و پرداخت چک به تأمین‌کننده
- فیلدها: **شمارهٔ چک و تاریخ سررسید اجباری** · صیادی، صاحب حساب و بانک اختیاری · توضیحات اختیاری
- در سررسید چکِ دریافتی از کاربر پرسیده می‌شود: «چکی که از فلانی دریافت کردیم پاس شده؟» → وصول به حساب / برگشت خورد
- **یادآور چک‌های پرداختیِ خود کاربر: از ۷ روز قبل تا روز سررسید** (فقط یادآوری، بدون پرسش) در داشبورد
- مبلغ چک‌های صادره جزو پرداختی‌های فروشگاه
- مراکز: در جریان / وصول‌شده / برگشتی · برگشت چک بدهی را دقیقاً برمی‌گرداند

### 💳 فروش اقساطی (درگاه‌های اعتباری)
- درگاه‌ها: **اسنپ‌پی، ترب‌پی، دیجی‌پی، تارا، باسلام، مستقیم فروشگاه، سفارشی** — همه کاملاً قابل‌پیکربندی (کارمزد ٪/ثابت، مالیات کارمزد، کسورات، الگو و تعداد اقساط) از تنظیمات برنامه
- الگوهای تسویه: روز ۱–۵ ماه‌های بعد (ترب‌پی) · فاصلهٔ ثابت ۳۰/۶۰ روز (تارا) · درصد قسط اول + اقساط بعدی (باسلام؛ درصد همان لحظهٔ فروش انتخاب می‌شود)
- کارمزد ابتدا کسر و خالص بین اقساط تقسیم می‌شود؛ کل مبلغ فروش هرگز «نقد امروز» فرض نمی‌شود
- سررسید اقساط مشتری، اقساط معوق، **یادآور ماهانهٔ «درگاه تسویه کرد؟ چه مبلغی؟»**
- تسویهٔ قسط‌به‌قسط با سقف · لغو/برگشت با اثر معکوس دقیق
- گزارش هر درگاه: فروش، کارمزد، انتظار تسویه، تسویه‌شده، **طلب تسویه‌نشده** · **پیش‌بینی ماهانهٔ واریزهای آتی**

### 💸 هزینه‌ها و صندوق
- دسته‌های هزینه: بسته‌بندی، ارسال، خرید، اجاره، حقوق، آب‌برق، سایر + دستهٔ سفارشی
- خرید عمدهٔ بسته‌بندی هزینهٔ واقعی است و هرگز خودکار بین فاکتورها توزیع نمی‌شود؛ «دریافت بسته‌بندی از مشتری» رویداد جدا و قابل مقایسه است
- حساب‌های صندوق/بانک/کارت‌خوان با موجودی مشتق · انتقال وجه (نه درآمد نه هزینه) · واریز/برداشت
- **کادر توضیحات اختیاری در همهٔ دریافت‌ها و پرداخت‌ها**

### 📊 گزارش‌ها (۱۱ نوع)
سود و زیان (جدا از جریان نقدی) · فروش روزانه/ماهانه · جریان نقدی واقعی (افتتاحیه→اختتامیه) · پیش‌بینی نقدی آینده · سود کالا · بدهکاران · تأمین‌کنندگان · هزینه‌ها (دسته‌ای/ماهانه/بسته‌بندی) · اقساط و درگاه‌ها · موجودی انبار و گردش کالا · **موتور تطبیق مالی خودکار (۸ بررسی)** · بستن روز صندوق/بانک + تاریخچهٔ حسابرسی

### ⚙️ زیر ساخت مالی
- همهٔ مبالغ Long/تومان؛ تراکنشی و idempotent؛ اثر معکوس دقیقاً یک‌بار؛ هیچ تاریخ مالی حذف نمی‌شود
- دفتر کل یکپارچهٔ رویدادمحور با ۲۶ نوع رویداد (فروش، دریافت، چک، اقساط، هزینه و…)
- دیتابیس SQLite محلی نسخهٔ ۵ با مهاجرت‌های غیرمخرب
...
- نمایش وضعیت فاکتورها، مشتریان، کالاها و مانده‌حساب‌ها
- دسترسی سریع به ثبت فاکتور جدید و فهرست فاکتورها
...
## ماژول مدیریت فروشگاه (گسترش نسخهٔ ۲)

در کنار سیستم فاکتور موجود، یک ماژول کامل **مدیریت فروشگاه و حسابداری** آفلاین اضافه شده است (ورود: منوی کشویی داشبورد ← «مدیریت فروشگاه»):

- **خرید و تأمین‌کنندگان**: فاکتور خرید با تخفیف/ارسال/مالیات، ورود خودکار موجودی با بهای میانگین موزون، پرداخت و برگشت خرید با سقف مقدار خرید
- **هزینه‌ها**: دسته‌بندی (بسته‌بندی، ارسال، اجاره، حقوق، …)، ابطال ایمن با اثر معکوس، خرید عمدهٔ بسته‌بندی هرگز خودکار بین فاکتورها توزیع نمی‌شود
- **صندوق و بانک**: حساب‌های نقدی/بانکی/کارت‌خوان با موجودی مشتق از دفتر کل، انتقال وجه (بدون شمارش به‌عنوان درآمد/هزینه)
- **فروش اقساطی**: موتور عمومی با سیستم‌های قابل‌پیکربندی (اسنپ‌پی، ترب‌پی، دیجی‌پی، باسلام، اقساط مستقیم فروشگاه و سفارشی)؛ کارمزد/مالیات/تأخیر تسویه طبق قرارداد شما، برنامهٔ اقساط، سررسید/معوق، تسویهٔ خالص و گزارش هر سیستم
- **مالی مشتری**: ماندهٔ مشتق از دفتر کل، دریافت، برگشت وجه، سقف اعتبار اقساط مستقیم
- **گزارش‌ها**: سود و زیان (جدا از جریان نقدی)، فروش، جریان نقدی واقعی، پیش‌بینی نقدی، سود کالا، بدهکاران، تأمین‌کنندگان، هزینه‌ها، اقساط، موجودی انبار و موتور تطبیق مالی
- **بستن روز و حسابرسی**: تطبیق صندوق/بانک + تاریخچهٔ کامل عملیات

معماری، جداول و قوانین مالی این ماژول در `ARCHITECTURE.md` و `DATABASE.md` توضیح داده شده‌اند. تست‌های واحد مالی در `test/store/` قرار دارند و در CI اجرا می‌شوند.
