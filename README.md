...
### ۸. مدیریت کدهای رهگیری ارسال («کدهای رهگیری ارسال»)
امکان جدید عملیاتی Premium: ثبت گروهی اطلاعات ارسال ده‌ها سفارش از یک صفحه
(ورودی: تب «بیشتر»).
- **هر ORDER یک ردیف** (هرگز گروه‌بندی به‌صورت مشتری نیست): نام مشتری (خودکار
  از relation سفارش — برای فاکتور خرید: نام تأمین‌کننده) + «فاکتور #N» +
  **کد رهگیری** (String؛ leading zero/حروف/خط‌فاز دقیقاً حفظ می‌شوند) +
  **تاریخ ارسال** (جلالی، قابل ویرایش) + **روش ارسال** (از همان ShippingProvider
  فاز ۳: پست/تیپاکس/پیک/سایر). **آدرس مشتری اصلاً در این صفحه نیست.**
- جستجوی زنده «جستجوی نام مشتری...» در سطح دیتابیس (JOIN + LIKE).
- فیلترها: همه / ارسال نشده / کد رهگیری ثبت نشده / ارسال شده.
- ترتیب پیش‌فرض: اول سفارش‌هایی که کد رهگیری ندارند، بعد بدون پیک، بعد جدیدترین.
- **ذخیره گروهی**: دکمهٔ «ثبت اطلاعات ارسال» فقط ردیف‌های **تغییرداده‌شده** را در
  **یک تراکنش Room** ذخیره می‌کند (partial bulk save)؛ هر خطا = rollback کامل.
- پس از ذخیره، همان داده روی Order می‌نشیند؛ فاکتور/جزئیات سفارش همان
  اطلاعات را می‌بیند — هیچ ورودی دوم نیست. (در UI فاکتور روبی بخشی جدید
  اختراع نشده؛ داده فقط متصل است.) **توجه:** ثبت کد رهگیری وضعیت سفارش را
  عوض **نمی‌کند** — دادهٔ رهگیری و status مفاهیم جدا هستند (اسپک هیچ رابطه‌ای
  بین آن‌ها تعریف نکرده؛ تست رگرسیونی دارد).
- **ارسال کد رهگیری** (SMS): کامپوزر پیامک اندروید با شمارهٔ ذخیره‌شدهٔ مشتری +
  متن خودکار؛ بدون شماره → «شماره موبایل مشتری ثبت نشده است.» + «ویرایش مشتری».
- **اشتراک‌گذاری**: Share Sheet بومی اندروید (بدون hardcode اپ).
- **چندانتخاب**: چک‌باکس هر ردیف + «اشتراک‌گذاری کدهای رهگیری» — برای هر
  سفارشِ انتخابی یک پیام جدا (هرگز ترکیب مشتری‌ها در یک پیام).
- لایه‌ها: `ShipmentTrackingRepository` (تراکنشی) + use case ها
  `SaveShipmentTrackingUseCase` / `BulkSaveShipmentTrackingUseCase` /
  `SearchOrdersForShipmentUseCase` / `GenerateTrackingMessageUseCase`
  (قالب دقیق: «{نام} عزیز،\nسفارش شما تحویل {پیک} شد.\nکد رهگیری: {کد}»).
- دیتابیس `version 5`: فقط دو ستون additive روی `orders`
  (`trackingCode String?`, `shippedAt Long?`) — دادهٔ موجود از دست نمی‌رود.
- تست‌ها: `ShipmentTrackingTest` (۱۲ تست: جستجو، چندسفارشی‌بودن مشتری،
  حفظ کد با صفرهای ابتدایی/حروف/خط‌فاز، provider/تاریخ، استقلال ردیف‌ها،
  bulk/partial bulk، **rollback تراکنش**، آپدیت، بدون-موبایل، نام تأمین‌کننده)
  + `GenerateTrackingMessageUseCaseTest` (۵ تست: قالب دقیق، پست/تیپاکس،
  صفرهای ابتدایی، جدا بودن پیام‌ها).
