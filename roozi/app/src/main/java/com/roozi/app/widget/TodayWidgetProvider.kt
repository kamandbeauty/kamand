...
class TodayWidgetProvider : AppWidgetProvider() {
        ...
        if (intent.action == ACTION_TOGGLE) {
            ...
            CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
                try {
                    ...
                    val current = com.roozi.app.data.local.RooziDatabase
    ...
    private fun openAppIntent(context: Context, quickAdd: Boolean = false): PendingIntent {
        val intent = Intent(context, MainActivity::class.java).apply {
            ...
            action = if (quickAdd) "roozi.QUICK_ADD" else Intent.ACTION_MAIN
        }
    ...
    companion object {
        const val ACTION_TOGGLE = "com.roozi.app.widget.TOGGLE"
